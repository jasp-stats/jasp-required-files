...
if len(sys.argv) > 2:
	...
	print("Use the '-no_path_change' option to disable executable path changes" )
print("Skipping executable path changes: ", skip_path_changes)

printLots = False

# run otool -L on libFile and return an array of libs that are loaded by libFile, but only if they start with "/Library/Frameworks/R.framework/", "/opt/" or "/usr/local/"
...
# locate_libs will go through all directories and files in a given directory, doing so recursively. It will remove certain folders deemed unnecessary (help, html, demo, tests and .dSym
# If it finds *.so and *.dylib it will use otool to find any other libs needed by the file in question and then calls locate_libs on the lines starting with /opt/, /usr/local/ or /Library/Frameworks/R....
def locate_libs(path):
	...
	if os.path.isdir(path):
		...
		if len(locations) > 0 and printLots:
			print('Locations found in lib "' + path + '" are: ' + ", ".join(locations))

	return locations

#Takes all "dependencies" from libs, where dependencies are basically all libraries being loaded that are also found by getOtoolFiles
...
#Go through the collected dependencies and make sure they are moved to out_lib_dir
#Also add these new locations to changes where the "old" portion is both the full path and the base.
...
#run change_dep_paths on all new_libs after setting the proper ID in the lib (because they were copied to another location) with installNameToolID		
...
#This is where most of the magic happens, the new_path's for each lib are determined and set as ID through installNameToolID and then the paths to their dependencies are changed based on what is contained in changes
for lib in libs:
